...
const skip = ({ start, end, testPath }) => toTestResult({
  stats: {
    failures: 0,
    pending: 1,
    passes: 0,
...
const fail = ({ start, end, testPath, errorMessage }) => toTestResult({
  errorMessage,
  stats: {
    failures: 1,
    pending: 0,
...
const pass = ({ start, end, testPath }) => toTestResult({
  stats: {
    failures: 0,
    pending: 0,
    passes: 1,
...
const runESLint = ({ testPath, config }, workerCallback) => {
  try {
    ...
    const CLIEngine = _getLocalESLint.CLIEngine;
